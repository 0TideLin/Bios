...
EFI_STATUS
EFIAPI
UsbViewAppInitialize(
  IN EFI_HANDLE         ImageHandle,
  IN EFI_SYSTEM_TABLE   *SystemTable
){
  EFI_STATUS Status;
  // SHELL_STATUS ShellStatus;
  mUsbViewHiiHandle = InitializeHiiPackage(ImageHandle);
  if(mUsbViewHiiHandle == NULL){
    Print(L"Hii Null\n");
    return EFI_ABORTED;
  }

  Status = RunUsbView(ImageHandle, SystemTable);

  HiiRemovePackages(mUsbViewHiiHandle);
  return Status;
}
